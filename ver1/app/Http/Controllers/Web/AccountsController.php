...
class AccountsController extends Controller
{
    ...
    public function pullAirlines(\App\Http\Requests\FTRequest $request)
    {
        ...
        try {
            ...
            if($airlines->count()>0)
            {
                ...
                foreach ($airlines as $airline) {
                    $accts[$airline->id] = $airline->airlineName;
                }
                return response()->json(['airlines' => $accts]);
            }else{
    ...
    /***pul Departures By Airline***/
    public function pullDeparturesByAirline(\App\Http\Requests\FTRequest $request) {
        $token = JWTAuth::getToken();
        ...
        try {
            ...
            $airlineflights = \DB::table('airlineflights')->join('airlinecity', 'airlineflights.departureCityId', '=', 'airlinecity.id')
                ->where('airlineflights.airlineId', '=', $airline)
                ->where('airlineflights.domesticYes', '=', $flighttype)
                ->groupBy('airlineflights.departureCityId')
            ...
            if($airlineflights->count()>0)
            {
                ...
                return response()->json(['token' => $token,'airlineflights' => $accts]);
            }else{
                return response()->json(['err' => 'No departure cities were found'], 500);
            }
